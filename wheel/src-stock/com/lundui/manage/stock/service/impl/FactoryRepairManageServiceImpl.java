...
import com.lundui.manage.stock.dao.FactoryRepairDao;
...
public class FactoryRepairManageServiceImpl implements
		FactoryRepairManageService {

	@Resource(name = "factoryRepairDao")
	private FactoryRepairDao factoryRepairDao;
	...
	public FactoryRepair addFactoryRepair(FactoryRepair entity) {
		return factoryRepairDao.saveEntity(entity);
	}

	@Override
	public PageModel<FactoryRepair> findFactoryRepair(String jcNum,
			Date dateFrom, Date dateTo, String depotCode) {
		...
		return this.factoryRepairDao.findPageModel(crits, Order
				.desc("repairDate"));
	}

	@Override
	public FactoryRepair addFactoryRepair(User user, Depot depot,
			FactoryRepair entity) {
		...
		if (entity.getSrcRecord1() != null) {
			...
			// 先添加一个轮对支出
			destRecord = wheelRecordDao.getEntity(entity.getDestRecrord1()
					.getId());
		...
		if (entity.getSrcRecord2() != null) {
			...
			// 先添加一个轮对支出
			destRecord = wheelRecordDao.getEntity(entity.getDestRecrord2()
					.getId());
			addIncomAndOutlay(user, record, destRecord, depot);
		}
		if (entity.getSrcRecord3() != null) {
			...
			// 先添加一个轮对支出
			destRecord = wheelRecordDao.getEntity(entity.getDestRecrord3()
					.getId());
			addIncomAndOutlay(user, record, destRecord, depot);
		}
		if (entity.getSrcRecord4() != null) {
			...
			// 先添加一个轮对支出
			destRecord = wheelRecordDao.getEntity(entity.getDestRecrord4()
					.getId());
			addIncomAndOutlay(user, record, destRecord, depot);
		}
		return this.factoryRepairDao.saveEntity(entity);
	}
	 ...
	 * @param depot
	 */
	private void addIncomAndOutlay(User user, WheelRecord record,
			WheelRecord destRecord, Depot depot) {
